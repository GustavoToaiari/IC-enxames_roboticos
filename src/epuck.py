...
def main():
    ...
    while True:
        ...
        for rob in robots:
            ...
            x, y = pose[0], pose[1]
            yaw = yaw_from_quaternion(pose[3], pose[4], pose[5], pose[6]) + math.pi/2
            rob['pose'] = (x, y, yaw)
            rob['traj'].append((x, y))
    ...
    ax.set_xlabel('x [m]')
    ax.set_ylabel('y [m]')
    ax.set_title(f'Trajetórias de {N} ePucks (campo potencial + repulsão mútua)')
    ax.axis('equal')
    ax.grid(True)
    ax.legend()
    plt.tight_layout()
